...
void ShowMsgBox(HWND hwnd, MsgboxCallback cb, MsgBoxType type,
	...
	const std::wstring &no, bool btn_no_is_id)
{
	...
	msgbox->SetButton(btn_yes_is_id ? multilan->GetStringViaID(yes) : yes, btn_no_is_id ? multilan->GetStringViaID(no) : no, type);
	msgbox->Show(hwnd, cb);
}
...
void MsgBox::InitWindow()
{
	...
	type_image_ = (Control*)FindControl(L"typeimage");
}
...
void MsgBox::SetButton(const std::wstring &yes, const std::wstring &no, MsgBoxType type)
{
	if (!yes.empty())
	{
	...
	{
	case ui::MT_INFOR:
		btn_yes_->SetClass(L"btn_global_green_80x30");
		type_image_->SetBkImage(L"infor.png");
		break;
	case ui::MT_QUESTION:
		btn_yes_->SetClass(L"btn_global_blue_80x30");
		type_image_->SetBkImage(L"question.png");
		break;
	case ui::MT_WARNING:
		btn_yes_->SetClass(L"btn_global_red_80x30");
		type_image_->SetBkImage(L"warning.png");
		break;
	case ui::MT_ERROR:
		btn_yes_->SetClass(L"btn_global_red_80x30");
		type_image_->SetBkImage(L"error");
		break;
	default:
		break;
	}
